...
#region Helpers
        ...
        public ActionResult GetMyProfile()
        {
            ...
            return View("~/Views/Manage/MyProfile.cshtml", viewModel);
        }

        [HttpPost]
        public ActionResult SaveMyProfile(AppUserViewModel viewModel)
        {
            try
            {
                var userId = User.Identity.GetUserId();
                ...
                //imgDto.FileContentType = viewModel.FileContentType;
                //imgDto.FileAttach = viewModel.FileAttach;

                //FileLib.GetFileFromFileAttach(imgDto);

                //user.FileContentType = imgDto.FileContentType;
                //user.Avatar = imgDto.FileContent;
        ...
        private bool ChangePassword(AppUserViewModel usermodel)
        {
        ...
        public ActionResult DownloadFile()
        {
            ...
            var imgDto = new ImgDto();
            imgDto.FileContent = user.Address;
            imgDto.FileContentType = user.FileContentType;

            try
            {
                // Info.
                return this.GetFile(imgDto);
            }
            catch (Exception ex)
            {
                // Info
            ...
            // Info.
            return this.View(imgDto);
        }

        private FileResult GetFile(ImgDto imgDto)
        {
            // Initialization.
            ...
            {
                // Get file.
                byte[] byteContent = Convert.FromBase64String(imgDto.FileContent);
                file = this.File(byteContent, imgDto.FileContentType);
            }
            catch (Exception ex)
            {
                // Info.
        ...
        public ActionResult UploadFiles()
        {
            ...
            if (Request.Files.Count > 0)
            {
                try
                {
                    ...
                    if (result.Succeeded)
                    {
                        // return Redirect("/Manage/GetMyProfile");
                        return Json("File Uploaded Success!");
                    }
                    else
                    {
